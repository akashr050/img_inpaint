...
flags.DEFINE_string('checkpoint_dir', "checkpoint", 'Path to evaluation csv')
flags.DEFINE_string('train_file', 'train.txt', 'Path to evaluation csv')
flags.DEFINE_string('inp_dir', "data", 'Path to input directory')
...
def get_train_paths(train_file):
  img_paths = []
  data_path = os.path.join(os.getcwd(),FLAGS.inp_dir)
  with open(train_file) as train_data:
    while train_data.readline():
      image_name = train_data.readline()
      image_name = data_path+image_name
      img_paths.extend(image_name)
  img_paths = np.array(img_paths)
  return img_paths


def train_glc():
  #################Checking if tb_dir exists##################################
  if not os.path.isdir(os.path.join(os.getcwd(),FLAGS.tb_dir)):
    os.mkdir(os.path.join(os.getcwd(),FLAGS.tb_dir))
  #################Checking and making the checkpoint directory ##############
  chkpt_path = os.path.join(os.getcwd(),FLAGS.checkpoint_dir)
  if not os.path.isdir(chkpt_path):
    os.mkdir(chkpt_path)
  ############################################################################
  train_img_paths = get_train_paths(os.path.join(os.getcwd(),FLAGS.train_file))
    ...
    while True:
      try:
        for counter in range(T_TRAIN):
          ...
          if step%10 == 0:
            saver.save(sess, chkpt_path+"/model", global_step=step)
            eval(sess,inputs,tb_writer)
          if counter < T_C:
